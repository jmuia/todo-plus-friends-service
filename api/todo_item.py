...
class CreateTodoItemHandler(BaseHandler):
...
class TodoItemHandler(BaseHandler):
    def get(self, username, list_id, item_id):
...
routes = [
    (r'/users/(\w+)/todo-lists/(\d+)/items/'     , CreateTodoItemHandler),
    (r'/users/(\w+)/todo-lists/(\d+)/items/(\d*)', TodoItemHandler      ),
]
